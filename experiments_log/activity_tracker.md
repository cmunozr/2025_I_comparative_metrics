...
- [X] **Configure Model 16 (M16) as Baseline**
...
- [X] **Execute the Southern Training / Northern Prediction Test**
  - Train the model exclusively on data from the South, then predict species occurrences/abundances in the North to verify if predictive relationships are indeed significantly worse up north.
    - August 11th, i made a mistake with the hold out and north-south tests. Re-running
    - August 17th, the level of goodness of fit for AUC is high with some species with bad fit and keeps north-south but for SR2 is low in overall with a bunch of species in the quite low rank.
- [ ] **Create the "Excluding the North" Model Filter**
  - Set up a filter to restrict the control-treatment dataset, removing the northern region (Lapland/specific ElYý regions).
...
- [ ] **Run prediction over the no-north region**
  ...
  - Calculate species richness and aggregate abundance across all species. Do not run individual gradient analyses for each separate species to keep the final paper focused .
- [ ] **Run metrics**
...
- [X] **Integrate Species Classification List**
