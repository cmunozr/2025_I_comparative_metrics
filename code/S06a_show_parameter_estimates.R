...
for (i in 1:nrow(run_config$mcmc)) {  
  ...
  if (file.exists(model_path)) {
    ...
    cov_map <- data.frame(model_cov = cov_names_model, var_target = cov_names_model) |>
      dplyr::left_join(dplyr::select(dict, var_target, type, Inline_Abbreviated), by = "var_target") |>
    ...
    if (m$nc > 1) {
      ...
      covariate_index_key <- data.frame(
        ...
        Model_Covariate = cov_map_sub$model_cov,
        Domain = cov_map_sub$type,
        Inline = cov_map_sub$Inline_Abbreviated
      )
      
      write.csv(covariate_index_key, paste0(estimates_dir, "/", "covariate_key_table.csv"), row.names = F)
      ...
      gt_key <- covariate_index_key |>
        ...
        cols_label(
          Plot_Index = "Index", Model_Covariate = "Model Covariate", Domain = "Domain"
        ) |>
        cols_align(
          align = "center", columns = c(Plot_Index, Domain)
        ) |>
        cols_align(
          align = "left", columns = Model_Covariate
        ) |>
        opt_row_striping() |>
        tab_options(
