...
validation_strategies <- "north_south"
...
for(i in 1:nrow(mcmc_params)){
  ...
  for(strategy in validation_strategies) {
    ...
    # --- 4.4 Evaluate Model Fit at the Site (Route) Level ---
    message("    Evaluating Site-Level Model Fit for ", label)
    
    # Calculate the posterior mean of the predictions across all MCMC samples
    predY_mean <- apply(predY, c(1, 2), mean, na.rm = TRUE)
    Y_obs <- hM_full$Y
    
    # Identify distribution families (1 = normal, 2 = Probit PA, 3 = log poisson)
    distr_types <- hM_full$distr[, 1]
    pa_idx <- which(distr_types == 2)
    cont_idx <- which(distr_types == 3)
    
    n_sites <- nrow(Y_obs)
    site_AUC <- rep(NA, n_sites)
    site_SR2 <- rep(NA, n_sites)
    
    # Loop through each site to calculate community-level spatial metrics
    for (i in 1:n_sites) {
      # Skip sites not included in the current cross-validation fold
      if (all(is.na(predY_mean[i, ]))) next 
      
      # 1. Calculate AUC for Presence/Absence species at site i
      if (length(pa_idx) > 0) {
        y_pa_observed <- ifelse(Y_obs[i, pa_idx] > 0, 1, 0)
        y_pa_predicted <- predY_mean[i, pa_idx]
        
        # AUC is only mathematically defined if the site has both presences and absences
        if (length(unique(y_pa_observed)) > 1) {
          # Suppress messages from pROC to keep the console clean
          site_AUC[i] <- suppressMessages(
            as.numeric(pROC::roc(y_pa_observed, y_pa_predicted, direction = "<")$auc)
          )
        }
      }
      
      # 2. Calculate SR2 (Spearman Rank R-squared) for Continuous/Count species at site i
      if (length(cont_idx) > 0) {
        y_cont_observed <- Y_obs[i, cont_idx]
        y_cont_predicted <- predY_mean[i, cont_idx]
        
        # Calculate Spearman correlation and square it
        spearman_cor <- cor(y_cont_observed, y_cont_predicted, method = "spearman", use = "complete.obs")
        site_SR2[i] <- spearman_cor^2
      }
    }
    
    # Bind the site-level metrics to the study design dataframe to preserve spatial structure
    # This directly maps the fitness metrics to the coordinates of the fixed-route transects
    spatial_fitness_df <- cbind(hM_full$dfPi, AUC = site_AUC, SR2 = site_SR2)
    spatial_fitness_df <- spatial_fitness_df[!is.na(spatial_fitness_df$AUC) | !is.na(spatial_fitness_df$SR2), ]
    
    # Save the resulting spatial dataframe
    saveRDS(spatial_fitness_df, file = mf_output_path)
    message("    Successfully saved spatial site-level evaluation to: ", mf_output_path)
    
    # Clean memory before transitioning to the next strategy
    rm(predY, predY_mean, Y_obs, spatial_fitness_df, site_AUC, site_SR2, distr_types, pa_idx, cont_idx)
    gc()
  }
}
