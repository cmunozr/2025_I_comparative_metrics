...
unique(occurr_biotope_route$vakio) |> length()
```
...
Chunk to add coordinates of each centroid of polygon biotope as covariates 
```{r}
sf_use_s2(TRUE)
...
studyDesign_clean <- studyDesign|>
  mutate(biotope = str_extract(sampleUnit, "[A-Z]$")) 
...
```{r}
# Distribution of the basic sampling units (biotopes)
biotope_stats <- studyDesign_clean|>
  count(biotope)|>
...
biotope_stats|>
  gt()|>
  tab_header(title = "Distribution of Forest Biotopes")|>
  fmt_number(columns = prop, decimals = 1)|>
  cols_label(prop = "Percentage (%)")
```
...
```{r}
# Average number of distinct biotopes? and How many biotopes (segments) per route?
route_composition_stats <- studyDesign_clean %>%
  group_by(vakio, year) %>%
  summarise(
    n_distinct_types = n_distinct(biotope), # e.g., How many types (K, M, S...) found?
    n_total_segments = n()                  # e.g., How many rows (data points) per route?
  ) %>%
  ungroup() %>%
  summarise(
    mean_distinct_types = mean(n_distinct_types),
    sd_distinct_types   = sd(n_distinct_types),
    mean_segments       = mean(n_total_segments),
    sd_segments         = sd(n_total_segments)
  )

route_composition_stats %>%
  gt() %>%
  tab_header(title = "Route Heterogeneity & Sampling Density") %>%
  cols_label(
    mean_distinct_types = "Avg. Distinct Biotope Types",
    sd_distinct_types = "SD (Types)",
    mean_segments = "Avg. Total Segments",
    sd_segments = "SD (Segments)"
  ) %>%
...
```{r}
# In average how many times a biotope in a route was surveyed?
# Assuming that "vakio 53 - Spruce" is the same spatial entity across years
biotope_repetition <- studyDesign_clean %>%
  mutate(stable_id = paste(vakio, biotope, sep = "_")) %>% 
  count(stable_id, name = "years_surveyed")

biotope_repetition_stats <- biotope_repetition %>%
  summarise(
    mean_years = mean(years_surveyed),
    sd_years = sd(years_surveyed),
    min_years = min(years_surveyed),
    max_years = max(years_surveyed)
  )

biotope_repetition_stats %>%
  gt() %>%
  tab_header(title = "Temporal Stability of Biotope Units") %>%
  cols_label(
    mean_years = "Avg. Years Surveyed",
    sd_years = "SD",
    min_years = "Min",
    max_years = "Max"
  ) %>%
  fmt_number(columns = c(mean_years, sd_years), decimals = 2)
...
survey_trend %>%
  ...
  tab_header(
    ...
    subtitle = "Number of Routes and Biotope Segments Surveyed per Year"
  ) %>%
  cols_label(
    year = "Year",
    n_routes = "Routes Surveyed",
    n_biotopes = "Biotope Segments"
  ) %>%
  fmt_number(columns = c(n_routes, n_biotopes), decimals = 0)
```
...
m <- Hmsc(
  ...
  ranLevels = list("vakio" = rL.locality, "year" = rL.year)
)
```
