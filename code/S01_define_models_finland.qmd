...
if(run_download_mactavish){
  get_avesdata_repo(path = file.path("data")) # 1GB more or less
}
...
if(run_calculate_XData | run_new_var_XData){
  ...
  if(run_new_var_XData){
    ...
    if (length(conflicting_targets) > 0) {
      message("Following variables already exist in XData and will be skipped: ", 
              paste(conflicting_targets, collapse = ", "))
      ...
      unique_vars_to_process <- setdiff(not_processed, vars_to_skip)
      
      if(length(unique_vars_to_process) == 0){
        
        stop("There is no additional variable to add in XData, skipping. 
             Check associated variables dictionary or if you want to re-run all covariates use run_calculate_XData = T")
      }
      
    }else{
      unique_vars_to_process <- not_processed
    }
    
  }else{
    unique_vars_to_process <- unique_vars
  }
  ...
  for (var_name in unique_vars_to_process) {
    ...
    if(raw_data$dataset[1] == "clim"){
      var_by_polygon <- var_by_polygon |>
        mutate(year = ifelse(str_split(polygon_id, pattern = "_", simplify = T )[,2] == year, "_yr", "_yr_last" )) |>
        pivot_wider(id_cols = polygon_id, 
                    names_from = year, 
                    values_from = plan_for_this_file$var_target, 
                    names_prefix = plan_for_this_file$var_target
        ) 
      }
    ...
    all_results_list[[var_name]] <- var_by_polygon |>
      ungroup() |> 
      select(-dplyr::any_of("year"))
  }
  
  if(run_new_var_XData){
    
    new <- all_results_list |> 
      purrr::compact() |> 
