...
```{r} 
# late Spring/Summer routes of years 2009, 2011, 2013, 2015, 2017, 2019, 2021 (years because the MS-NFI)

ms_nfi_years <- c(2009, 2011, 2013, 2015, 2017, 2019, 2021)

routes <- routes |>
  mutate(out_date = if_else(
    condition = month < 5 | month > 7 | ! (year %in% ms_nfi_years),
    true = 1,
    false = 0
  ))
...
# Filtering routes and occurrences in routes
...
rm(index_odo, current_data, current_df, n.r, cols, high_rank, info, tax_raw)
```
...
# how many routes do not have biotope or subsection info each year?
...
rm(no_biotope, no_biotope_routes, no_biotope_years, no_biotope_table, sample_by_year, percent_sample_no_biotope)
```
...
occurr_sect_route |> 
  group_by(year, vakio) |> 
  summarise(total_sections = n(), sections_missing_biotope = sum(biotope == "")) |> 
  filter(sections_missing_biotope != 0) |> 
  mutate(vakio = as.factor(vakio)) |> 
  pivot_longer(cols = c(total_sections, sections_missing_biotope), names_to = "category", values_to = "rows") |> 
  ...
  scale_fill_discrete(
    labels = c(
      "sections_missing_biotope" = "Sections with Missing Biotope", 
      "total_sections" = "Total Sections"
    )
  )

```
How the missing biotope routes are spread in the country? Is some area prone to have less data?

```{r}
# joining to spatial data "times missing biotope" information
...
rm(lvls, turbo_discrete_colors, custom_colors)
```
Is some route without no target species information?
...
occurrence_data <-  table(scientificName = occurr_sect_route$scientificName, biotope_logic = occurr_sect_route$biotope_logic) |>
  ...
  rename(no_biotope_occurrences = `0`, 
         biotope_occurrences = `1`
         ) |>
  mutate(
    total_occurrences = (no_biotope_occurrences + biotope_occurrences),
...
rm(occurrence_data, paircount_data)
```
...
occurr_sect_route <- bind_cols(occurr_sect_route, number_strip)
...
### Extracting information from Covariates
...
main_data_dir <- "D:/intermediate_aggregated"
...
all_rds_files <- list.files(main_data_dir, pattern = "\\.rds$", recursive = TRUE, full.names = TRUE)
...
for (covariate in dict_covar) {
  ...
  # writing a filtered data set just to coords because metso is a sample
  ...
  is_binary <- all(unique(plot_data$value) %in% c(0, 1))
