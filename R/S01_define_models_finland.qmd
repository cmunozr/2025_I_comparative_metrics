...
if(run == T){
  ...
  for(i in 1:nrow(spp_in_route)){
    ...
    tax_in_route_raw[[i]] <- data.frame("taxonId" = resp.i$id, "inputName" = spp.i, "outputName" = resp.i$scientificName, "englishName" = resp.i$vernacularName$en,
                                        "taxonRank" = resp.i$taxonRank, "is.species" = resp.i$species, "is.finnish" = resp.i$finnish,
                                        "group" = resp.i$informalGroups[[1]]$name$en) 
  }
  
  tax_in_route_raw <- tax_in_route_raw |> 
    bind_rows() |> 
...
tax_in_route <- tax_in_route_raw |>
  filter(is.species == TRUE, group == "Birds", taxonRank != "MX.hybrid", taxonRank != "MX.form") |>
  mutate(outputName = ifelse(taxonRank == "MX.subspecies",
                                        stringr::word(outputName, 1, 2),
                                        outputName
                                        )
         ) |>
  select(
    original_route_name = inputName,
    accepted_finbif_name = outputName,
    taxonId
  ) |>
  distinct()
```
...
# Loop through each species in the Bakx et al. list
for (i in 1:nrow(bakx_species)) {
  
  spp.i <- bakx_species$Latin_name[i]
  
  resp.i <- tryCatch({
    finbif::finbif_taxa(spp.i, type = "exact")$content[[1]]
  }, error = function(e) {
    NULL
  })
  
  if (!is.null(resp.i)) {
    tax_bakx_raw[[i]] <- data.frame(
      original_bakx_name = spp.i,
      accepted_finbif_name = resp.i$scientificName,
      taxonId = resp.i$id
    )
  } else {
    tax_bakx_raw[[i]] <- data.frame(
      original_bakx_name = spp.i,
      accepted_finbif_name = NA,
      taxonId = NA
    )
  }
}

tax_in_bakx <- bind_rows(tax_bakx_raw)
...
harmonized_species <- harmonized_species |>
  mutate(
    in_routes_data = !is.na(original_route_name),
    in_bakx_list = !is.na(original_bakx_name),
    acceptedName = ifelse(!is.na(accepted_finbif_name_routes), 
                           accepted_finbif_name_routes, 
                           accepted_finbif_name_bakx)
  ) |>
  select(
    taxonId,
    acceptedName,
...
```{r}
table("routes" = harmonized_species$in_routes_data, "bakx" = harmonized_species$in_bakx_list)
```
We are missing the next birds from Bakx et al (2023):

```{r}
harmonized_species$original_bakx_name[!harmonized_species$in_routes_data]
...
target_species <- harmonized_species |> 
  filter(in_routes_data == T, in_bakx_list == T) |> 
  pull(acceptedName)
...
phyloTree <- extractTree(species = spp_Y.new, version = 1.4)

phyloTree$tip.label <- spp_Y[match(str_replace(phyloTree$tip.label, "_", " ") , spp_Y.new)]
...
spp_Y.new <- data.frame("scientificName" = spp_Y.new)

TrData <- avonet |> 
  rename(scientificName = Species2) |>
  right_join(spp_Y.new, by = "scientificName" ) |>  
