...
```{r}
biotope_L3_paths <- tidyr::expand_grid(
  vakio = coords$vakio,
  year = ms_nfi_years
) |>
  mutate(
...
for (y in 1:length(ms_nfi_years)) {
  ...
  for (a in 1:nrow(coords)) {
    ...
    log_row_index <- which(biotope_L3_paths$vakio == route_id & biotope_L3_paths$year == current_year)
    ...
    if(any(biotope_route_a$r_startMeters > 1)){
      ...
      biotope_L3_paths$was_extended[log_row_index] <- TRUE
      biotope_L3_paths$extension_distance_m[log_row_index] <- d
    
    }else{
      biotope_L3_paths[a,"extended"] <- F
    }
    
    if (nrow(biotope_route_a) > 1) {
      
      biotope_L3_paths$was_processed[log_row_index] <- TRUE
      ...
      for (r in 1:nrow(biotope_route_a)) {
        ...
        route_buffers[[r]] <- final_buffer_sf
          ...
          point_strip_a <- biotope_L3_paths$point_strips[a]
          biotope_L3_paths$point_strips[a] <- c(point_strip_a, biotope_route_a$biotopeStripId[r])
          warning("Some biotope observations are points")
        }
  ...
  biotope_L3_paths[biotope_L3_paths$year == current_year, "path"] <- output_filename
