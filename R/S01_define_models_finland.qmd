...
if(run_process_covariates){
  ...
  output_rds_paths <- NA_character_
  ...
  for (i in 1:length(covar_nm)) {
    ...
    covar_data_i <- lapply(relevant_files, readRDS) |> 
      bind_rows() # it binds metso and route data, can be big, watch out
    file_path <- file.path(folder_name,  paste0(covar_nm_i, ".rds"))
    saveRDS(covar_data_i, file = file_path)
    
    output_rds_paths[i] <- file_path
    
    if(plotting){ # not fixed with the new structure
      plot_data <- prepare_covariates_data_toplot(
        filtered_data = covar_data_i
      )
  ...
  output_rds_paths <- na.omit(output_rds_paths)
...
ggplot(vif_plot_data, aes(x = Variable, y = VIF)) +
  geom_col() +
  geom_hline(yintercept = vif_threshold, color = "red", linetype = "dashed", size = 0.5) +
  labs(
