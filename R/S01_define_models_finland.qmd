...
# occurrences by track (spp key, and counts in points and lines)

occurr_track <- read.csv("data/fbs/vakiolinja/vakiolinja2_20250626.csv") 

# occurrences by section track (spp key, and counts in points and lines)

occurr_sect_track <- read.csv("data/fbs/vakiolinja/vakiolinja3_20250626.csv") 


```

```{r}
# exploring biotope information

no_biotope <- occurr_sect_track |> 
  filter(biotope == "")
no_biotope <- (table(no_biotope$vakio, no_biotope$year) > 0) 

# a) what percentage of tracks have no biotope information or subsection information
# b) of those without information how many times have been without biotope info?

no_biotope_tracks <- no_biotope |> 
  rowSums()

## a 
length(no_biotope_tracks)/566*100

## b
table(no_biotope_tracks)/566*100

no_biotope_tracks_df <- data.frame(
  vakio = names(no_biotope_tracks),
  times_missing = no_biotope_tracks
)

# how many tracks have no had biotope or subsection info each year?
# what percentege of tracks that were sample each year dont have biotope or subsection info?

no_biotope_years <- no_biotope |> 
  colSums()

sample_by_year <- occurr_sect_track |> 
  filter(year != 2019) |> 
  group_by(year) |> 
  summarize(sample_tracks = length(unique(vakio)))

percent_sample_no_biotope <- no_biotope_years/sample_by_year$sample_tracks*100

hist(percent_sample_no_biotope)

# how many of the biotope and subsection information is missing? Is all the track?
...
coords <- bind_cols(coords, coords_nm) |> 
  left_join(y = no_biotope_tracks_df, by = "vakio") |> 
  mutate(vakio = sort(as.numeric(vakio)), times_missing = as.factor(ifelse(is.na(times_missing), 0, times_missing)))
