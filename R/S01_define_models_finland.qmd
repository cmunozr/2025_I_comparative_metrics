...
### Aggregate biotope information from vakio+biotope+biotopeSpecifier to vakio+biotope. The former is too finer. 
...
occurr_biotope_route <- occurr_sect_route |>
  group_by(vakio, year, biotope, scientificName) |>
  summarise(
...
Y <- occurr_biotope_route |> 
  select(sampleUnit, scientificName, pairCount) |>
  pivot_wider(id_cols = sampleUnit, names_from = scientificName, values_from = pairCount,
...
We have to resolve names used in the phylogeny database and the bird survey.(BirdTree database)\[https://birdtree.org/\] The database comes from: (Jetz, W., G. H. Thomas, J. B. Joy, K. Hartmann, and A. O. Mooers. 2012. The global diversity of birds in space and time. Nature 491:444-448)\[https://www-nature-com.ludwig.lub.lu.se/articles/nature11631\]. The species list from the database were web scrapped. There is no API, so write and retrieve manually. BirdTree names will be an anchor for trait database names.

```{r}
spp_Y <- colnames(Y)

bird_tree <- birdTreeSpp$species

is_missing_birdTree <- !(spp_Y %in% bird_tree)

spp_resolved <- data.frame("scientificName" = NA_character_, "bird_tree" = NA_character_)
...
if(run){
  ...
  for(i in 1:length(spp_Y)){
    ...
    if(is_missing_birdTree[i]){
        
        syn_spp_i <- taxize::synonyms(spp_i, db = "nbn", rec_only = T, accepted = F, rank = "species", ask = F) |> 
          map_df(~ .x)
        
        if(ncol(syn_spp_i) == 1 | ncol(syn_spp_i) == 0){
          ...
          syn_spp_i <-  syn_spp_i |> 
            select(nameString) |> 
            pull()
          syn_spp_i <- (syn_spp_i[syn_spp_i %in% birdTreeSpp$species]) |> 
            unique()
          }
    }else{
        syn_spp_i <- spp_i
  ...
  spp_resolved[i,"scientificName"] <- spp_i
  spp_resolved[i,"bird_tree"] <- syn_spp_i
  }
  
  spp_resolved <- bind_cols(spp_resolved, is_missing_birdTree) |> 
    rename(missing_birdTree = 3)
  write.csv(spp_resolved, "data/bird_tree/taxonomical_synonyms_birdtree.csv", row.names = F)
  
}else{
  spp_resolved <- read.csv("data/bird_tree/taxonomical_synonyms_birdtree.csv")
}
```
