...
coords |> 
  ...
  scale_color_manual(
    ...
    labels = c("TRUE" = "Information Species", "FALSE" = "No Information Species"),
...
years_to_process <- unique(occurr_sect_route$year) |> sort()
...
for (current_year in years_to_process) {
  ...
  for (a in 1:nrow(coords)) {
    ...
    if (nrow(biotope_route_a) > 0) {
      ...
      split_route_list_year[[a]] <- do.call("rbind", split_route_i) |> 
  ...
  if (length(split_route_list_year) > 0) {
    ...
    output_paths <- c(output_paths, output_filename)
