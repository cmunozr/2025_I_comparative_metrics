...
if(run == T){
  ...
  tax_in_route_raw <- tax_in_route_raw[-missing_index]
...
to_rename <- taxize::synonyms(spp_Y[not_in_mctavish], db = "itis")[[1]] |> 
  select(acc_name) |> 
  pull() |> 
  unique()

# "Sylvia curruca" change to "'Curruca curruca"

spp_Y[which(spp_Y == spp_Y[not_in_mctavish])] <- to_rename

phyloTree <- extractTree(species = spp_Y, version = 1.4)
...
spp_Y <- data.frame("scientificName" = colnames(Y))

TrData <- avonet |> 
  rename(scientificName = Species2) |>
  right_join(spp_Y, by = "scientificName" ) |>  
