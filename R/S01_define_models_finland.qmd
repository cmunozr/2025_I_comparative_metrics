...
master_routes <- routes |>
  ...
  left_join(
    occurr_sect_route |>
      filter(is_biotope_forest == TRUE) |> # this flag sums is.species, group == birds, in_bakx
      distinct(vakio, year) |>
...
if(run){
  ...
  XData <- all_results_list |> 
    ...
    right_join(y = distinct(select(occurr_biotope_route, sampleUnit)), join_by("polygon_id" == "sampleUnit")) |> 
    rename("sampleUnit" = "polygon_id")
...
#| fig-height: 10
#| 
corr_matrix <- cor(na.omit(XData[,-1])) ################# WATCHOUT SOME ARE NA, NO COVER FOR MSI-NFI ############
col_palette <- colorRampPalette(c("#4477AA", "#FFFFFF", "#BB4444"))(200)

corrplot::corrplot(
  corr_matrix, 
  type = "upper",
  method = "color",
  col = col_palette,        # Apply our custom color palette
  addCoef.col = "black",    # Add the correlation coefficients in black text
  tl.col = "black", 
  tl.cex = 0.6, 
  number.cex = 0.6
)
