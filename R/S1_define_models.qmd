...
write.csv(xy.karta, "xy.karta.csv", row.names = F)
...
model <- "swedishBirds_forestry_small"
modeltype <- c("abundance_small")

dir.create("models", showWarnings = F)
models <-  list(m.abu)
names(models) <- paste0(model, "_", modeltype)
save(models, file = file.path("models", "unfitted_models_small.RData"))
...
```{r}
model <- "swedishBirds_forestry"
modeltype <- c("abundance", "abuCpres", "presenceAbscence")
...
```{r}
sampleMcmc(m_unfitted, samples=2)
```
